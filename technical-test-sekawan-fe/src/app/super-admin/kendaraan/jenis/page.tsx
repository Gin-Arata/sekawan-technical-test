...
export default function JenisKendaraanPage() {
  ...
  const fetchJenisKendaraan = async () => {
    try {
      const response = await fetch(`${apiUrl}/super-admin/jenis-kendaraan`, {
        method: "GET",
        headers: {
  ...
  const handleAddDataSubmit = (e: any) => {
    handleSubmit(e, `${apiUrl}/super-admin/jenis-kendaraan`, "POST");
  };

  const handleEditDataSubmit = (e: any) => {
    handleSubmit(
      e,
      `${apiUrl}/super-admin/jenis-kendaraan/${selectedJenisKendaraan?.id}`,
      "PATCH"
    );
  };
