...
export default function DaerahPage() {
  ...
  const [history, setHistory] = useState<HistoryPenyewaan[] |  null>([]);
  ...
  const fetchData = async () => {
    try {
      const [
        ...
        fetch(`${apiUrl}/manager/pending-sewa`, {
          method: "GET",
          headers: {
  ...
  return (
    <div className="p-2">
      <h1 className="text-3xl font-bold mb-4">Terima Penyewaan</h1>
      ...
      <Modal
        isOpen={addModalEdit}
        onClose={closeModal}
        title={`Penyewa ${user.find((u) => u.id === selectedHistory?.user_id)?.name}`}
      >
        <div>
